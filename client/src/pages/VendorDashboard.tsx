...
import { Package, Upload, FileSpreadsheet, Plus, Loader2, Trash2 } from "lucide-react";
      ...
      <main className="container max-w-4xl mx-auto px-4 py-8">
        ...
        {tab === "upload" && (
          <div className="bg-white rounded-xl border border-border/50 p-6 space-y-4 max-w-md">
            ...
            <Button onClick={handleManualUpload} disabled={createProduct.isPending} className="w-full" data-testid="button-add-product">
              {createProduct.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Upload className="w-4 h-4 mr-2" />}
              Add Product
            </Button>
          </div>
        )}
