...
import { ShoppingCart, Menu, Home, User, Settings, Truck } from "lucide-react";
    ...
    <>
      <header className="sticky top-0 z-50 w-full border-b bg-white/80 backdrop-blur-md hidden md:block">
        <div className="container max-w-7xl mx-auto flex h-16 items-center justify-between px-4">
          ...
          <nav className="flex items-center gap-6">
            ...
            <Link href="/vendor-dashboard"><span className="text-sm font-medium hover:text-primary cursor-pointer" data-testid="link-vendor">Vendor</span></Link>
            <Link href="/staff-dashboard"><span className="text-sm font-medium hover:text-primary cursor-pointer" data-testid="link-staff">Staff</span></Link>
            <Link href="/admin-dashboard"><span className="text-sm font-medium hover:text-primary cursor-pointer" data-testid="link-admin">Admin</span></Link>
          </nav>
          ...
          <Sheet>
            ...
            <SheetContent>
              <div className="flex flex-col gap-4 mt-8">
                ...
                <Link href="/vendor-dashboard" className="text-lg font-medium">Vendor Dashboard</Link>
                <Link href="/staff-dashboard" className="text-lg font-medium">Staff Dashboard</Link>
                <Link href="/admin-dashboard" className="text-lg font-medium">Admin Dashboard</Link>
              </div>
            </SheetContent>
          </Sheet>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t z-50 md:hidden pb-safe">
        <div className="flex justify-around items-center h-16">
          ...
          <NavLink href="/vendor-dashboard" icon={User} label="Vendor" />
          <NavLink href="/cart" icon={ShoppingCart} label={`Cart (${cartCount})`} />
          <NavLink href="/staff-dashboard" icon={Truck} label="Staff" />
          <NavLink href="/admin-dashboard" icon={Settings} label="Admin" />
        </div>
      </nav>
